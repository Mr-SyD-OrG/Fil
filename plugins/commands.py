...
async def syd_callback(client: Client, cb: CallbackQuery):
    ...
    if data.startswith("syd_view|"):
        _, trig = data.split("|", 1)
        doc = await db.get_filter(trig)

        text = f"""Trigger: <code>{doc['trigger']}</code>
Response: {doc.get("response_text") or "<empty>"}"""

        kb = [
            [InlineKeyboardButton("Send", callback_data=f"syd_send|{trig}")],
    ...
    if data.startswith("syd_send|"):
        ...
        if doc.get("photo_file_id"):
            await client.send_photo(chat_id, doc["photo_file_id"], caption=doc.get("response_text") or "", reply_markup=markup)
        else:
    ...
    if data.startswith("syd_remove|"):
        _, trig = data.split("|", 1)
        await db.delete_filter(trig)
        await cb.message.edit_text("Deleted.", reply_markup=syd_main_kb())
        return

    if data == "syd_delete_all":
        await db.delete_all_filters()
        await cb.message.edit_text("All filters deleted.", reply_markup=syd_main_kb())
        return
    ...
    if data == "syd_add":
        ...
        if btn_msg.text and btn_msg.text.lower() != "/skip":
            button_text = btn_msg.text if btn_msg.text != "/default" else "ϟ  Oᴩᴇɴ Cʜᴀɴɴᴇʟ  ϟ"
...
async def handle_add_flow(client, chat_id):

    await client.send_message(chat_id, "Send trigger:")
    trigger_msg = await client.ask(chat_id)
    trigger = trigger_msg.text.strip()

    await client.send_message(chat_id, "Send response text or /skip:")
    resp_msg = await client.ask(chat_id)
    response_text = "" if resp_msg.text == "/skip" else resp_msg.text

    await client.send_message(chat_id, "Send button text or /skip:")
    btn_msg = await client.ask(chat_id)

    button_text = None
    button_url = None

    if btn_msg.text != "/skip":
        button_text = btn_msg.text
        await client.send_message(chat_id, "Send button URL:")
        url_msg = await client.ask(chat_id)
        button_url = url_msg.text

    await client.send_message(chat_id, "Send photo or /skip:")
    photo_msg = await client.ask(chat_id)
    photo_file_id = photo_msg.photo.file_id if photo_msg.photo else None

    await db.create_filter_doc(trigger, response_text, button_text, button_url, photo_file_id)
